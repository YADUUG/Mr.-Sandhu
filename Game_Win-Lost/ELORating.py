...
class CheatDetector:
    def __init__(
        self, headshot_threshold=0.75, accuracy_threshold=0.80, kd_ratio_threshold=20.0
    ):
    ...
    def _load_and_process_data(self, data_file):
        ...
        for match_id, group in df.groupby("Match_ID"):
            ...
            for player in team_a_players:
                player.rating = self.elo.update_rating(
                    player.rating, expected_a, actual_a
                )
            for player in team_b_players:
                player.rating = self.elo.update_rating(
                    player.rating, expected_b, actual_b
                )

    def _calculate_final_team_ratings(self):
        for team in self.teams.values():
            member_ratings = [
                self.players[username].rating
...
if __name__ == "__main__":
    ...
    if full_leaderboard:
        for rank, player_info in enumerate(full_leaderboard, 1):
            print(
                f"#{rank:<3} | {player_info['username']:<15} | Rating: {player_info['rating']:.2f}"
            )
    print("\n" + "=" * 50 + "\n")
    ...
    if all_teams:
        for team_info in all_teams:
            print(
                f"- Team Name: {team_info['team_name']:<20} | Average Rating: {team_info['rating']:.2f}"
            )
    print("\n" + "=" * 50 + "\n")
